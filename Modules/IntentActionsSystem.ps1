...
$global:IntentAliases += @{
    ...
    "schedule_workflow"         = {
        ...
        $errorLog = "$env:TEMP\bildsyps_task_errors.log"
        ...
        $scriptPath = Join-Path $scriptsDir "BildsyPS_$workflow.ps1"
        
        # Build bootstrap script content with absolute paths
        $scriptContent = @"
# Auto-generated by BildsyPS schedule_workflow -- do not edit
...
try {
    ...
    "[`$(Get-Date)] BILDSYPS TASK ERROR ($workflow): `$(`$_.Exception.Message)" | Out-File `$errorLog -Append
...
"@
        ...
        # 4. Register Task under \BildsyPS folder
        $taskFolder = "\BildsyPS"
        $taskName = "Workflow_$workflow"
        $description = "BildsyPS automated workflow '$workflow' ($schedule)"
    ...
    "list_scheduled_workflows"  = {
        ...
        try {
            ...
            if (-not $tasks) {
                return @{ Success = $true; Output = "No scheduled workflows found in \BildsyPS\" }
            }
    ...
    # ===== Vision =====
    "analyze_image"             = {
        param($path, $prompt, $full)
        if (-not $path) {
            return @{ Success = $false; Output = "Error: path parameter required"; Error = $true }
        }
        if (-not (Get-Command Send-ImageToAI -ErrorAction SilentlyContinue)) {
            return @{ Success = $false; Output = "VisionTools module not loaded"; Error = $true }
        }
        ...
        $fullRes = $full -eq 'true' -or $full -eq $true
        $result = Send-ImageToAI -ImagePath $path -Prompt $prompt -FullResolution:$fullRes
        return $result
    }
    ...
    "screenshot"                = {
        ...
        if (-not $capture.Success) {
            return @{ Success = $false; Output = "Screenshot failed: $($capture.Output)"; Error = $true }
        }
